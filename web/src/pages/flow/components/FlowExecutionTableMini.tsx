import { Link, Popup, Table, TableProps, TableRowData, Tag } from "tdesign-react";
...
export interface FlowExecutionTableMiniRef {
  requestList: () => void;
}

interface FlowExecutionTableMiniProps {
  uid?: string;
  parentStepId?: string;
  // updateBtn: boolean;
  // showExpand?: boolean;
...
const FlowExecutionTableMini = forwardRef<FlowExecutionTableMiniRef, FlowExecutionTableMiniProps>((props, ref) => {
  ...
  const columns: TableProps["columns"] = [
    ...
    // { colKey: "Creator", title: "创建人", width: 100, ellipsis: true },
    {
      ...
      cell: ({ row }) => row.Input && <JsonLink json={row.Input} title="输入" />,
    },
    {
      colKey: "Output",
      title: "输出",
      ...
      cell: ({ row }) => row.Output && <JsonLink json={row.Output} title="输出" />,
    },
    // {
    //   colKey: "Context",
    //   title: "上下文",
    ...
    },
    {
      ...
      width: 100,
      fixed: "right",
      cell: ({ row }) => <UpdateExecutionDialog execution={row as FlowExecution} onUpdate={() => requestList()} />,
    },
  ];
  ...
  function requestList() {
    ...
    if (props.uid) {
      params["Uids"] = [props.uid];
    }
    if (props.parentStepId) {
      params["ParentStepId"] = props.parentStepId;
    }
    FlowApi.DescribeFlowExecutions(params).then((resp) => {
      if (resp.Data) {
...
export default FlowExecutionTableMini;
